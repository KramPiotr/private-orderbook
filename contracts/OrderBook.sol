...
contract OrderBook is ReentrancyGuard {
    ...
    function placeBuyOrder(
        ...
        for (uint8 orderIdx = 0; orderIdx < N_ORDERS; orderIdx++) {
            ...
            euint8 potentialQtyFilled = FHE.min(
                qtyLeft,
                sellBook[orderIdx].qty
            );
            euint8 qtyFilled = FHE.select(
                isCrossing,
                potentialQtyFilled,
                CONST_0_ENCRYPTED
            );
            ...
            shiftBy = FHE.select(
                sellBook[orderIdx].qty.eq(CONST_0_ENCRYPTED),
                incrementedShiftBy,
                shiftBy
            );
            ...
            lastFills[orderIdx] = ExecutionResult(
                sellBook[orderIdx].id,
                qtyFilled
            ); //doing this hack as transferrring caused issues
        }
    ...
    function shiftSellBook() external {
        ...
        for (uint8 shiftIdx = 0; shiftIdx < N_ORDERS - 1; shiftIdx++) {
            ...
            for (uint8 orderIdx = 0; orderIdx < N_ORDERS - 1; orderIdx++) {
                sellBook[orderIdx].id = FHE.select(
                    doShift,
                    sellBook[orderIdx + 1].id,
                    sellBook[orderIdx].id
                );
                sellBook[orderIdx].price = FHE.select(
                    doShift,
                    sellBook[orderIdx + 1].price,
                    sellBook[orderIdx].price
                );
                sellBook[orderIdx].qty = FHE.select(
                    doShift,
                    sellBook[orderIdx + 1].qty,
                    sellBook[orderIdx].qty
                );
            }

            sellBook[N_ORDERS - 1].id = FHE.select(
                doShift,
                CONST_0_ENCRYPTED,
                sellBook[N_ORDERS - 1].id
            );
            sellBook[N_ORDERS - 1].price = FHE.select(
                doShift,
                CONST_0_ENCRYPTED,
                sellBook[N_ORDERS - 1].price
            );
            sellBook[N_ORDERS - 1].qty = FHE.select(
                doShift,
                CONST_0_ENCRYPTED,
                sellBook[N_ORDERS - 1].qty
            );
        }
    }
    ...
    function insertBuyOrder(
        ...
        for (uint8 orderIdx = 0; orderIdx < N_ORDERS; orderIdx++) {
            ebool isCorrectPosition = FHE.or(
                buyBook[orderIdx].price.eq(CONST_0_ENCRYPTED),
                FHE.and(price.gte(prevPrice), buyBook[orderIdx].price.gt(price))
            );
            shift = FHE.or(shift, FHE.and(doInsert, isCorrectPosition));
            prevPrice = buyBook[orderIdx].price;

            euint8 nextIdCarriedOn = FHE.select(
                shift,
                buyBook[orderIdx].id,
                idCarriedOn
            );
            euint8 nextPriceCarriedOn = FHE.select(
                shift,
                buyBook[orderIdx].price,
                priceCarriedOn
            );
            euint8 nextQtyCarriedOn = FHE.select(
                shift,
                buyBook[orderIdx].qty,
                qtyCarriedOn
            );

            buyBook[orderIdx].id = FHE.select(
                shift,
                idCarriedOn,
                buyBook[orderIdx].id
            );
            buyBook[orderIdx].price = FHE.select(
                shift,
                priceCarriedOn,
                buyBook[orderIdx].price
            );
            buyBook[orderIdx].qty = FHE.select(
                shift,
                qtyCarriedOn,
                buyBook[orderIdx].qty
            );
    ...
    function placeSellOrder(
        ...
        for (uint8 orderIdx = 0; orderIdx < N_ORDERS; orderIdx++) {
            ...
            euint8 incrementedShiftBy = shiftBy + CONST_1_ENCRYPTED;
            shiftBy = FHE.select(
                buyBook[orderIdx].qty.eq(CONST_0_ENCRYPTED),
                incrementedShiftBy,
                shiftBy
            );
    ...
    function shiftBuyBook() external {
        ...
        for (uint8 shiftIdx = 0; shiftIdx < N_ORDERS - 1; shiftIdx++) {
            ...
            for (uint8 orderIdx = 0; orderIdx < N_ORDERS - 1; orderIdx++) {
                buyBook[orderIdx].id = FHE.select(
                    doShift,
                    buyBook[orderIdx + 1].id,
                    buyBook[orderIdx].id
                );
                buyBook[orderIdx].price = FHE.select(
                    doShift,
                    buyBook[orderIdx + 1].price,
                    buyBook[orderIdx].price
                );
                buyBook[orderIdx].qty = FHE.select(
                    doShift,
                    buyBook[orderIdx + 1].qty,
                    buyBook[orderIdx].qty
                );
            }

            buyBook[N_ORDERS - 1].id = FHE.select(
                doShift,
                CONST_0_ENCRYPTED,
                buyBook[N_ORDERS - 1].id
            );
            buyBook[N_ORDERS - 1].price = FHE.select(
                doShift,
                CONST_0_ENCRYPTED,
                buyBook[N_ORDERS - 1].price
            );
            buyBook[N_ORDERS - 1].qty = FHE.select(
                doShift,
                CONST_0_ENCRYPTED,
                buyBook[N_ORDERS - 1].qty
            );
        }
    }
    ...
    function insertSellOrder(
        ...
        for (uint8 orderIdx = 0; orderIdx < N_ORDERS; orderIdx++) {
            ebool isCorrectPosition = FHE.or(
                sellBook[orderIdx].price.eq(CONST_0_ENCRYPTED),
                FHE.and(
                    price.lte(prevPrice),
                    sellBook[orderIdx].price.lt(price)
                )
            );
            shift = FHE.or(shift, FHE.and(doInsert, isCorrectPosition));
